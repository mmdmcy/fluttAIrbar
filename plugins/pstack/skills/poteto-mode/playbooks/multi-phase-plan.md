...
3. Explore in subagents with `a pstack-style subagent` and an explicit model per the Subagents section (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
- [ ] Arm the 30-minute audit tick. In a local session, a real terminal `the host recurring-monitoring workflow`. In a cloud root, a cloud-sleeper wake chain. Never leave the cadence to memory.
...
Each live lane runs on its own cloud VM at the PR head. Drive through `host UI verification tooling` or `host CLI verification tooling` from `available host tooling`.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes on `auto` at the PR head, per the boot recipe.
