...
This skill orchestrates three others: an inline mining pass (see step 1), Codex's `$skill-creator` skill (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `.codex/skills/**/*-mode/SKILL.md` and `~/.codex/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.codex/skills/<handle>/`), not only at the top level. If one exists, confirm intent with `structured user-choice prompt` (unless they already said "update my skill" or similar):
...
Locate the active workspace's transcripts before fanning out. The system prompt names the workspace's `agent-transcripts/` directory. Use only that path. Don't glob across `~/.codex/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Mining misses intent that hasn't come up yet. Use the `structured user-choice prompt` tool (structured multi-choice) rather than asking the user to type from scratch. Lower cognitive load, higher hit rate.
...
Use Codex's `$skill-creator` skill skill to author the skill. Placement:
...
- Codex metadata `allow_implicit_invocation: false` by default. Mode skills are heavy and opinionated; they should only apply when the user explicitly invokes them (by name or slash command), not auto-trigger on description matching. Opt out only if the user explicitly wants their mode to apply on every turn.
...
- Codex's `$skill-creator` skill skill: skill authoring process and writing guidelines.
