...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, current editor location, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
- `model`: your configured why-investigators model (default `auto`)
...
- subagent type: general-purpose
- `model`: your configured why-synthesizer model (default `auto`)
